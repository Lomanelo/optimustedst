...
export default function CreateBlogPostPage() {
  ...
  const updateSlugPreview = async (value: string) => {
    ...
    if (sanitized) {
      try {
        const available = await blogService.isSlugAvailable(sanitized);
  ...
  useEffect(() => {
    // If user hasn't provided a custom SEO title yet, mirror the main title up to 60 chars
  ...
  // --- Lightweight Markdown Toolbar helpers ---
  const wrapSelection = (
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Create a new blog post for your website with multilingual support
        </p>
      </div>
      ...
      {success && (
        <div className="mb-6 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-md flex items-start">
          ...
          <div>
            <p className="font-medium">Blog post created successfully!</p>
        ...
        <div className="flex justify-end">
          ...
          >
            {isSubmitting ? (
              ...
              status === 'published' ? 'Publish Blog Post' : 'Save as Draft'
            )}
          </button>
        </div>
      </form>
